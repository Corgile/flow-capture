...
void Stats::print_stats() const {
    ...
    fprintf(stderr, "  Packets skipped:   %10llu (%.2f%%)\n",
            (unsigned long long)packets_skipped,
            100.0 * (double(packets_skipped) / packets_processed));
    fprintf(stderr, "  Packets parsed:    %10llu (%.2f%%)\n",
            (unsigned long long)packets_parsed,
            100.0 * (double(packets_parsed) / packets_processed));
...
void Stats::update(bool parsed, uint8_t network_layer,
    ...
    if (!parsed) {
        ...
        if (transport_layer == IPPROTO_TCP)
            tcp++;
        if (transport_layer == IPPROTO_UDP)
            udp++;
        if (transport_layer == IPPROTO_ICMP)
            icmp++;
    }
}
