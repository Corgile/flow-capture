...
void TCPHeader::fill_bit_vec(std::vector<int8_t> &vec, int8_t bit) {
  uint32_t tcp_header_byte_size, zero_byte_width;
  
  if (raw == nullptr) {
    make_bitstring(SIZE_TCP_HEADER_BITSTRING, nullptr, vec, bit);
    return;
  }
  tcp_header_byte_size = raw->th_off * 4;
  zero_byte_width = SIZE_TCP_HEADER_BITSTRING - tcp_header_byte_size;
  make_bitstring(tcp_header_byte_size, (void *) raw, vec, bit);
  make_bitstring(zero_byte_width, nullptr, vec, bit);
}

void TCPHeader::gen_bit_header(std::vector<std::string> &vec) {
