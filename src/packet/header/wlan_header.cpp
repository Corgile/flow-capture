...
uint32_t WlanHeader::header_len() {
  ...
  return SIZE_WLAN_HEADER_BITSTRING;
}

void WlanHeader::fill_bit_vec(std::vector<int8_t> &vec, int8_t bit) {
  make_bitstring(SIZE_WLAN_HEADER_BITSTRING, raw, vec, bit);
}
  ...
  switch (raw->type) {
    ...
    default:
      ...
      sprintf(buf,
              ...
              raw->tx_addr[0], raw->tx_addr[1],
              raw->tx_addr[2], raw->tx_addr[3],
              raw->tx_addr[4], raw->tx_addr[5]);
      return buf;
  }
}

void WlanHeader::gen_bit_header(std::vector<std::string> &vec) {
