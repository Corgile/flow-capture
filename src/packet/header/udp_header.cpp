...
void UDPHeader::fill_bit_vec(std::vector<int8_t> &vec, int8_t bit) {
  make_bitstring(SIZE_UDP_HEADER_BITSTRING, raw, vec, bit);
}

void UDPHeader::gen_bit_header(std::vector<std::string> &to_fill) {
  ...
  PacketHeader::gen_bit_header(v, to_fill);
}
