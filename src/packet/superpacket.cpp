...
bool SuperPacket::process_v4(void *pkt) {
  ...
  if (ipv4_header.get_ip_proto() == IPPROTO_TCP) {
    ...
    payload_data = ((u_char *)tcph + tcp_header.header_len());
    payload_len = ipv4_header.get_total_len() -
                (tcp_header.header_len() + ipv4_header.header_len());
    ...
    payload_data = ((u_char *)udph + udp_header.header_len());
    payload_len = ipv4_header.get_total_len() -
                (ipv4_header.header_len() + udp_header.header_len());
    ...
    payload_data = ((u_char *)icmph + icmp_header.header_len());
    payload_len = ipv4_header.get_total_len() -
                (ipv4_header.header_len() + icmp_header.header_len());
  } else {
    return false;
  }
  payload.set_raw(payload_data);
  payload.set_info(payload_len, max_payload_len);
...
bool SuperPacket::process_v6(void *pkt) {
  ...
  if (ipv6_header.get_ip_proto() == IPPROTO_TCP) {
    tcph = (struct tcphdr *)((u_char *)ipv6h + ipv6_header.header_len());
    tcp_header.set_raw(tcph);
    ...
    udph = (struct udphdr *)((u_char *)ipv6h + ipv6_header.header_len());
    udp_header.set_raw(udph);
    pload = ((u_char *)udph + 8);
    pload_len = ipv6_header.get_total_len() -
...
void SuperPacket::fill_bit_vec(Config *c, std::vector<int8_t> &vec) {
  if (c->radiotap == 1)
    radiotap_header.fill_bit_vec(vec, c->bit);
  if (c->wlan == 1)
    radiotap_header.fill_bit_vec(vec, c->bit);
  if (c->eth == 1)
    ethernet_header.fill_bit_vec(vec, c->bit);
  if (c->ipv4 == 1)
    ipv4_header.fill_bit_vec(vec, c->bit);
  if (c->ipv6 == 1)
    ipv6_header.fill_bit_vec(vec, c->bit);
  if (c->tcp == 1)
    tcp_header.fill_bit_vec(vec, c->bit);
  if (c->udp == 1)
    udp_header.fill_bit_vec(vec, c->bit);
  if (c->icmp == 1)
    icmp_header.fill_bit_vec(vec, c->bit);
  if (c->payload != 0)
    payload.fill_bit_vec(vec, c->bit);
}
