...
std::vector<std::string> FileWriter::build_bitstring_header(std::vector<std::string> header) {
  ...
  if (config.radiotap == 1)
    r.gen_bit_header(header);
  if (config.wlan == 1)
    w.gen_bit_header(header);
  if (config.eth == 1)
    e.gen_bit_header(header);
  if (config.ipv4 == 1)
    v4.gen_bit_header(header);
  if (config.ipv6 == 1)
    v6.gen_bit_header(header);
  if (config.tcp == 1)
    tcp.gen_bit_header(header);
  if (config.udp == 1)
    udp.gen_bit_header(header);
  if (config.icmp == 1)
    icmp.gen_bit_header(header);
  if (config.payload != 0)
    p.gen_bit_header(header);
...
void FileWriter::write_bitstring_line(std::vector<std::string> &prefix,
                                      std::vector<int8_t> &bit_vec) {
  
  for (const auto &i: prefix) {
    if (keep_indexes.empty()) {
      fprintf(outfile, "%s", i.c_str());
    } else {
      fprintf(outfile, "%s,", i.c_str());
    }
  }
  
  for (uint32_t i = 0; i < keep_indexes.size(); i++) {
    fprintf(outfile, "%d", bit_vec[keep_indexes[i]]);
